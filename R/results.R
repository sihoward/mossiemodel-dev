...
#' @param plasmod_devel Data frame output by
#'   \code{\link[mosqmod]{plasmod_devel}} used to plot shaded regions when
...
plot_popn <- function(resdf,
                      ...
                      include_temp = TRUE, plasmod_devel = NULL, MTT = NULL, MTD = NULL){
  ...
  if(is.data.frame(plasmod_devel)){

    # get temperature time series
    Tseq <- resdf[c("Date", "Tmean")]
    # check for non-contiguous dates and gaps
    if(any(diff(plasmod_devel$Date) != 1)) stop("non-contiguous dates in 'Date' field in plasmod_devel")

    # are thermal requirements met on each date?
    thermal_req <- plasmod_devel$thermal_req[match(Tseq$Date, plasmod_devel$Date)]
    ...
    # store NULL when plasmod_devel is not a data.frame
    gg_DEVRect <- NULL
    gg_DEVscale <- NULL
  }
